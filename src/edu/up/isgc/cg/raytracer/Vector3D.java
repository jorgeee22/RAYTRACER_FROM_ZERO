...
public class Vector3D {
    ...
    public static Vector3D reflect(Vector3D incident, Vector3D normal) {
        // r = d - 2*(d·n)*n
        double dotProduct = Vector3D.dotProduct(incident, normal);
        return Vector3D.substract(
                incident,
                Vector3D.scalarMultiplication(normal, 2 * dotProduct)
        );
    }

    public static Vector3D refract(Vector3D incident, Vector3D normal, double n1, double n2) {
        // Normaliza los vectores para evitar errores
        Vector3D incidentNorm = Vector3D.normalize(incident);
        Vector3D normalNorm = Vector3D.normalize(normal);

        double n = n1 / n2; // Ratio de índices de refracción
        double cosI = -Vector3D.dotProduct(normalNorm, incidentNorm);
        ...
        // t = n*d + (n*cosI - cosT)*n
        return Vector3D.add(
                Vector3D.scalarMultiplication(incidentNorm, n),
                Vector3D.scalarMultiplication(normalNorm, n * cosI - cosT)
        );
    }
}
